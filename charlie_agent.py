...
class CharlieAgent:
    ...
    def __init__(
        ...
        self._last_raw_action: str    = ""
        self._action_repeats: int     = 0
        self.successful_sequence: list[dict] = []
    ...
    async def run(self) -> dict:
        ...
        for step in range(1, self._max_steps + 1):
            ...
            # ── Inferencia contextual de paso del plan (Desactivado para Evitar Saltos) ─────────
            ...
            action_sig = f"{action.get('action')}_{action.get('selector', '')}_{action.get('value', '')}"
            if action_sig == self._last_raw_action and action.get("action") not in ("wait", "human_ask"):
                self._action_repeats += 1
            else:
                self._last_raw_action = action_sig
                self._action_repeats = 0
                
            if self._action_repeats >= 3:
                await self._log(f"⚠️ Bucle detectado ({self._action_repeats} iteraciones repetidas). Desviando.")
                self._needs_vision = True
                self._system_hint = "ERROR ESTRICTO: Estás atascado en un BUCLE repitiendo la misma acción sin efecto en la UI. Si no logras clickar el selector, ABANDONA ese selector. Haz scroll, avanza al siguiente paso del plan de inmediato usando 'advance_plan', o cancela y busca otra oferta."
                if self._action_repeats >= 5:
                    self._plan_idx += 1
                    self._action_repeats = 0
                continue
            ...
            # Record successful actions to build procedual Bot Skills
            if not self._needs_vision and action.get("action") in ("click", "fill", "press", "navigate", "done"):
                step_record = {
                    "action": action.get("action"),
                    "selector": action.get("selector"),
                    "value": action.get("value"),
                    "key": action.get("key"),
                    "url": self._last_url if action.get("action") == "navigate" else None
                }
                self.successful_sequence.append({k: v for k, v in step_record.items() if v is not None})
