...
@app.get("/api/status")
async def ollama_status():
    """Comprueba Ollama y qué modelos hay disponibles."""
    try:
        with urllib.request.urlopen(f"{OLLAMA_URL}/api/tags", timeout=3) as r:
            tags = json.loads(r.read())
        models = [m["name"] for m in tags.get("models", [])]
        configured_ok = any(OLLAMA_MODEL in m for m in models)
        return {
            "ollama":        "online",
            "model":         OLLAMA_MODEL,
            "model_present": configured_ok,
            "available":     models,
        }
    except Exception as e:
        return {"ollama": "offline", "error": str(e)}
...
async def preview_plan(body: dict):
    """
    NUEVO v3: genera y devuelve un plan sin arrancar el navegador.
    Útil para inspeccionar qué haría el planificador antes de ejecutar.
    Body: {"task": "...", "model": "...", "url": "about:blank", "login": "DESCONOCIDO"}
    """
    task       = body.get("task", "")
    model_name = body.get("model", OLLAMA_MODEL)
    url        = body.get("url", "about:blank")
...
async def websocket_endpoint(ws: WebSocket):
    ...
    try:
        ...
        model_name    = cfg.get("model", OLLAMA_MODEL).strip()
        task_override = cfg.get("task_override", "").strip() or None
        use_planner   = cfg.get("use_planner", True)   # Plan-and-Act activo por defecto
        ...
        mode_str = "Plan-and-Act" if use_planner else "Directo"
        await send("log",    f"🐾 Charlie v3 [{mode_str}] | {platform.capitalize()} | {role} | {salary}€")
        await send("log",    f"🤖 Modelo: {model_name}")
        if task_override:
            await send("log", f"🧪 Override: {task_override}")
        await send("status", "running")
        ...
        if task_override:
            if task_override == "google_test":
                task = (
                    "OBJETIVO: Busca 'Charlie agent test' en Google y reporta el título del primer resultado.\n"
                    "1. Navega a https://www.google.com\n"
                    "2. Usa 'fill' en el input 🌟 con el texto 'Charlie agent test'\n"
                    "3. Usa 'press' con 'Enter'\n"
                    "4. Lee el título del primer resultado con 'extract' y usa 'done'.\n"
                    "   job_url = URL del resultado | company = 'Google'"
                )
            else:
                task = task_override
...
Portal: {platform.capitalize()}
URL de búsqueda: {start_url}
Comprueba primero si estás logueado. Si no lo estás, usa 'error' de inmediato.

=== PASOS ===
1. Navega a la URL de búsqueda.
2. Selecciona la primera oferta relevante que NO hayas aplicado ya.
3. Abre el detalle haciendo clic en la oferta.
4. Pulsa el botón de solicitud:
   - LinkedIn → 'Solicitud sencilla' / 'Easy Apply'
   - Tecnoempleo / InfoJobs → 'Inscribirme en esta oferta'
5. Rellena el formulario (ver FORMULARIO).
6. Confirma el envío. Busca 'Enviar', 'Confirmar' o 'Submit'.
7. Usa 'done' con la URL exacta de la oferta y el nombre de la empresa.

=== FORMULARIO ===
- Salario deseado: {salary} (solo números)
- Nivel de inglés: B2 o C1
- Años de experiencia: 5
- Carta: "Hola, soy apasionado de la automatización con base en Física. Mi expectativa salarial es {salary} € brutos/año."
- Teléfono: si ya está relleno, déjalo. Si pide uno nuevo: 600000000.

=== IMPORTANTE ===
- NUNCA inventes URLs. Haz clic en el enlace de la oferta.
- NO uses 'done' hasta que la confirmación de envío sea visible.
"""
...
if __name__ == "__main__":
    import webbrowser
    print("🐾 Charlie Dashboard v3 en http://localhost:8080")
