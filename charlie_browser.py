...
class CharlieBrowser:
    ...
    async def get_element_at(self, x: int, y: int) -> dict:
        """Obtiene un selector básico y propiedades del elemento en coordenadas (x,y)."""
        if not self._page:
            return {}
        try:
            js = """
            ...
            """
            res = await self._page.evaluate(js, [x, y])
            return res or {}
        except Exception as e:
            await self._async_log(f"⚠️ Error get_element_at({x},{y}): {e}")
            return {}
